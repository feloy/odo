...
func (lo LogOptions) DevfileComponentLog() error {
	devObj, err := devfile.ParseFromFile(lo.GetDevfilePath())
	if err != nil {
		return err
	}
...
func (do *DeleteOptions) DevfileComponentDelete() error {
	devObj, err := devfile.ParseFromFile(do.GetDevfilePath())
	if err != nil {
		return err
	}
...
func (eo *ExecOptions) DevfileComponentExec(command []string) error {
	devObj, err := devfile.ParseFromFile(eo.componentOptions.GetDevfilePath())
	if err != nil {
		return err
	}
